...
def pair_variants(cogorts):
    pairs = []
    for comb in itertools.combinations(cogorts,2):
        prod = itertools.product(comb[0],comb[1])
        pairs = itertools.chain(pairs,prod)
    return(pairs)
...
def check_variants(procdata):
    variants = [0, 0, 0, 0]
    maxes = [[],[],[],[]]
    for j, vector in enumerate(procdata):
        maxcov = max(vector)
        maxes[getmax(vector)].append(j)
    ...
    return([maxes, max_alt, sum_of_alt])    
...
def read_all_files(index_handler, filename_list, logf, chrom_set, path,max_alt_treshold,sum_alt_treshold, suffix):
    ...
    mutualcov_dict = [dict_ini(chrom_set, filenum)]
    diffcov_dict = [dict_ini(chrom_set, filenum)]

    def dist_matrix_calculate(filenums_for_proc, data, chrom):
        procdata = [map(int, data[i]) for i in filenums_for_proc]
        #covs = [sum(el) for el in procdata]

        num_of_proc_files = len(procdata)
        maxes, max_alt,sum_of_alt = check_variants(procdata)
        ...
        if ((num_of_proc_files > 1) & max_alt_cond & sum_alt_cond):
            all_pairs = list(itertools.combinations(filenums_for_proc,2))
            all_pairs = sorted(all_pairs)
            for i, j in all_pairs:
                mutualcov_dict[0][chrom][i][j] += 1
            diff_pairs = pair_variants(maxes)
            for i, j in diff_pairs:
                ii = filenums_for_proc[i]
                jj = filenums_for_proc[j]
                diffcov_dict[0][chrom][ii][jj] += 1
        # except TypeError:
        #     print(filenums_for_proc)
        #     print(chrom)
        #     print(procdata)
        #     sys.exit()
    ...
    outf = open('%s/json.distance_matrices_%s.txt'  % (path, suffix), 'a')
...
def main(index_handler, filename_list_handler, chrom_list_handler, max_alt_treshold, sum_alt_treshold, suffix):
    ...
    outf = open('%s/json.distance_matrices_%s.txt' % (path, suffix), 'w')
    message = """
    ...
    that are covered in both samples.
    Structure of this dict:
    [{chrom:[[] * num_of_samples] * num_of_samples]}]
    --------------------------------------------------------------
    Third dictionary means number of positions
    where major letter are not similar.
    --------------------------------------------------------------
    """ % (filename_list_handler, filenum)
    ...
    read_all_files(index_handler, filename_list, logf, chrom_set, path,max_alt_treshold,sum_alt_treshold, suffix)
