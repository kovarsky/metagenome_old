...
def heterogen(vector):
    """Computes heterogenity at particular position"""
...
def read_all_files(index_handler, filename_list, logf, chrom_set, path,tresholds):
    ...
    mutualcov_dict = [dict_ini(chrom_set, filenum)] * len(tresholds)
    diffcov_dict = [dict_ini(chrom_set, filenum)] * len(tresholds)
    ...
    def dist_matrix_calculate(filenums_for_proc, data, chrom):
        ...
        if num_of_proc_files > 1:
            for i in xrange(num_of_proc_files - 1):
                for j in xrange(1, num_of_proc_files):
    ...
    outf = open('%s/json.distance_matrices.txt' % path, 'a')
...
def main(index_handler, filename_list_handler, chrom_list_handler, tresholds):
    ...
    outf = open('%s/json.distance_matrices.txt' % path, 'w')
    message = """
    ...
    read_all_files(index_handler, filename_list, logf, chrom_set, path,tresholds)
