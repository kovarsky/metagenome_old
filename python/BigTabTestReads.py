...
def check_variants(procdata):
    ...
    for vector in procdata:
        maxcov = max(vector)
        max_indices = [i for i in xrange(4) if vector[i] == maxcov]
...
def read_all_files(index_handler, filename_list, logf, chrom_set, path,tresholds,suffix):
    ...
    def dist_matrix_calculate(filenums_for_proc, data, chrom, cond):
        ...
        #sum_alt_cond = (sum_of_alt >= float(sum_alt_treshold))

        if ((num_of_proc_files > 1) & cond):
    ...
    for i, line in enumerate(index_file):
        ...
        chrom = stripped_line[0]
        position = stripped_line[1]
        reads_sum = float(stripped_line[6])
        alt_reads_sum = float(stripped_line[7])
        try:
            frac = alt_reads_sum / reads_sum
        except ZeroDivisionError:
            frac = 0
        cond = ((alt_reads_sum > 5) and (reads_sum > 300) and (frac > 0.01))
        ...
        dist_matrix_calculate(filenums_for_proc, data, chrom, cond)
...
def main(index_handler, filename_list_handler, chrom_list_handler, suffix, tresholds):
    ...
    read_all_files(index_handler, filename_list, logf, chrom_set, path,tresholds, suffix)
...
if __name__ == '__main__':
    if (len(sys.argv) < 2):
        print 'usage:\tBigTab3.py <in: sorted index file> <in: sorted *.tab filename list> <in: contig list>'
        sys.exit(0)
    else:
